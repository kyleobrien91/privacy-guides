...
def get_eui64():
  ...
  return "-".join([format(eui64, "02X")[i:i+2] for i in range(0, 18, 2)])
...
def main():
  ...
  prefix = ":".join((f"fd{globalid[:2]}", globalid[2:6], globalid[6:10]))
  print(f"Prefix:       {prefix}::/48")
  print(f"First subnet: {prefix}::/64")
  print(f"Last subnet:  {prefix}:ffff::/64")
